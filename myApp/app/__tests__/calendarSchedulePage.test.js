...
jest.mock('../screens/CalendarSchedulePage', () => {
  ...
  return () => (
    <>
      ...
      <Text testID="eventTime">{mockScheduleData[0].start} - {mockScheduleData[0].end}</Text>
      <TouchableOpacity testID="eventButton" onPress={() => mockEventPress(mockScheduleData[0])} />
    </>
  );
});
