...
export const addShuttleOption = async (origin, destination) => {
  ...
  if (
    !isLocationInCampus(destination, loyolaRegion) &&
    !isLocationInCampus(destination, sgwRegion)
  ) {
  ...
  const travelToShuttleOptions = await TravelFacade.getAlternativeRoutes(
    origin,
    shuttleStop,
    travelModes,
    1
  ); // Fetch only 1 best route per mode

  if (
    !travelToShuttleOptions ||
    Object.keys(travelToShuttleOptions).length === 0
  ) {
    return []; // No valid travel options to shuttle stop
  }

  // Step 4: Get shuttle time from SGW to Loyola (or Loyola to SGW)
  const shuttleInfo = await TravelFacade.estimateShuttleFromButton(
    isGoingToLoyola ? "SGW" : "LOY"
  );
  ...
  const shuttleDistance = TravelFacade.haversineDistance(sgwStop, loyolaStop);

  // Step 6: Modify each travel mode by adding shuttle time
  const shuttleOptions = Object.entries(travelToShuttleOptions).map(
    ([mode, routes]) => {
      ...
      const actualWaitTime = Math.max(
        0,
        shuttleInfo.nextShuttleTime - arrivalMinutes
      );

      return {
        mode: `${mode} + Shuttle`,
        duration:
          bestRoute.duration + actualWaitTime + shuttleInfo.shuttleRideTime,
        ...
        steps: [
          ...bestRoute.steps,
          {
            instruction: `Wait for shuttle at ${
              isGoingToLoyola ? "SGW" : "Loyola"
            }`,
            distance: "0 km",
            duration: actualWaitTime,
          },
          {
            instruction: `Take the shuttle to ${
              isGoingToLoyola ? "Loyola" : "SGW"
            }`,
